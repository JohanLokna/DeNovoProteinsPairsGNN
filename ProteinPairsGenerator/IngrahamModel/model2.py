...
class Struct2Seq(BERTModel):
    def __init__(self, 
        ...
        self.W_s = nn.Embedding(self.vocab_input, hidden_dim)
        ...
        self.W_out = nn.Linear(hidden_dim, vocab_output, bias=True)
    ...
    def forward_sequential(self, X, S, L, mask=None):
        ...
        log_probs = torch.zeros((N_batch, N_nodes, self.vocab_input))
    ...
    def step(self, x):
        ...
        for k in self.kAccuracy:
            ...
            print(yPred_k.shape, x.seq.shape)
            nCorrect_k = ((x.seq.unsqueeze(-1) == yPred_k) * x.mask).sum()
