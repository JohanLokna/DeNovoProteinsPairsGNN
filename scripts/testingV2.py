...
loader_train, loader_test = [dm.train_dataloader(), dm.test_dataloader()]

datasets = [StructureDataset(file, truncate=None, max_length=2000) for file in ["validation.jsonl"]]
loader_validation = dm.val_dataloader() # [StructureLoader(d, batch_size=batch_tokens) for d in datasets]
...
for e in range(epochs):
    ...
    with torch.no_grad():
        ...
        for x in tqdm(loader_validation):
    ...
    print('Perplexity\tTrain:{0: >#16.14f}\t\tValidation:{1: >#16.14f}'.format(0.0, validation_perplexity))
